...
const ChatRoomSettingsIcon = styled("svg", { shouldForwardProp: (props) => props !== "pressed" })(({ pressed, theme }) => ({
	transition: pressed ? "transform 0.3s" : "none",
...
const ChatRoomLog = ({ ownUsername, chatLog, selectedRoomObj, openRoomDetailsFunc, submitFieldValueFunc }) => {
	...
	const settingsButtonPressedRef = useRef(false);
	 ...
	 * @param {Object} e
	 */
	const onSettingsButtonPressed = (e) => {
		openRoomDetailsFunc(OVERLAYTYPES.ROOMDETAILS);
		settingsButtonPressedRef.current = true;
		setInterval(() => (settingsButtonPressedRef.current = false), 300);
	};
		...
		<ChatRoomLogContainer1>
			<ChatRoomTitleBar>
				...
				<IconButtonsContainer>
					...
					<ChatRoomSettingsIconButton disableRipple disableTouchRipple disableFocusRipple onClick={onSettingsButtonPressed}>
						<ChatRoomSettingsIcon pressed={settingsButtonPressedRef.current} style={{ width: "24px", height: "24px" }} viewBox="0 0 24 24">
							<path
